...
{
	float normalizationFactor = 1.0f / (2.0f * (_Radius * _Radius / PI));
	float distanceFactor = exp(-(distance * distance) / pow(_Radius / PI, 2) / 2);
...
float4 BlurGaussian(float2 uv, float2 direction)
{
	...
	float4 finalColor = lerp(centerColor, totalColor, _Factor);
